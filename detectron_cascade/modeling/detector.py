...
from detectron_cascade.ops.collect_and_distribute_fpn_rpn_proposals \
...
from detectron_cascade.ops.decode_bbox \
    import DecodeBBoxOp
from detectron_cascade.ops.generate_proposal_labels import GenerateProposalLabelsOp
from detectron_cascade.ops.generate_proposals import GenerateProposalsOp
...
class DetectionModelHelper(cnn.CNNModelHelper):
    ...
    def ConvGNShared(  # args in the same order of Conv()
        self, blob_in, prefix, dim_in, dim_out, kernel, stride, pad,
        group_gn,  # num of groups in gn
        conv_weight_init=None,
        conv_bias_init=None,
        suffix='_gn',
        gn_bias_init=None,
        gn_scale_init=None,
        ...
        """ConvGNShared op adds a ConvGN op that shares weights and/or biases with another ConvGN op,
        including learnable scale/bias (gamma/beta)
        """
        ...
        gn_bias = core.ScopedBlobReference(gn_bias_init)
        gn_scale = core.ScopedBlobReference(gn_scale_init)
        self.gn_params.append(gn_bias)  # add gn's bias to list
        self.gn_params.append(gn_scale)  # add gn's scale to list
        return blob_out
