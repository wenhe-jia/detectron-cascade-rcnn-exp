...
def get_cascade_fast_rcnn_blob_names(is_training=True, stage_num=1):
    ...
    if stage_num == 1:
        ...
        if is_training:
            ...
            blob_names += ['labels_int32_1st']
        if is_training:
            # bbox_targets blob: R bounding-box regression targets with 4
            # targets per class
            blob_names += ['bbox_targets_1st']
            # bbox_inside_weights blob: At most 4 targets per roi are active
            # this binary vector sepcifies the subset of active targets
            blob_names += ['bbox_inside_weights_1st']
            blob_names += ['bbox_outside_weights_1st']
            # add max overlaps of box to gt_box, to screen out high iou rois in
            # DecodeBBox operatot
            blob_names += ['max_overlaps_1st']
        if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_ROIS:
            ...
            for lvl in range(k_min, k_max + 1):
                blob_names += ['rois_1st_fpn' + str(lvl)]
            blob_names += ['rois_1st_idx_restore_int32']
    ...
    if stage_num == 2:
        ...
        if is_training:
            # labels_int32 blob: R categorical labels in [0, ..., K] for K
            # foreground classes plus background
            blob_names += ['labels_int32_2nd']
        if is_training:
            # bbox_targets blob: R bounding-box regression targets with 4
            # targets per class
            blob_names += ['bbox_targets_2nd']
            # bbox_inside_weights blob: At most 4 targets per roi are active
            # this binary vector sepcifies the subset of active targets
            blob_names += ['bbox_inside_weights_2nd']
            blob_names += ['bbox_outside_weights_2nd']
            # add max overlaps of box to gt_box, to screen out high iou rois in
            # DecodeBBox operatot
            blob_names += ['max_overlaps_2nd']
        if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_ROIS:
            # Support for FPN multi-level rois without bbox reg isn't
            # implemented (... and may never be implemented)
            k_max = cfg.FPN.ROI_MAX_LEVEL
            ...
            # Same format as rois blob, but one per FPN level
            for lvl in range(k_min, k_max + 1):
                blob_names += ['rois_2nd_fpn' + str(lvl)]
            blob_names += ['rois_2nd_idx_restore_int32']
    ...
    if stage_num == 3:
        ...
        if is_training:
            # labels_int32 blob: R categorical labels in [0, ..., K] for K
            # foreground classes plus background
            blob_names += ['labels_int32_3rd']
        if is_training:
            # bbox_targets blob: R bounding-box regression targets with 4
            # targets per class
            blob_names += ['bbox_targets_3rd']
            # bbox_inside_weights blob: At most 4 targets per roi are active
            # this binary vector sepcifies the subset of active targets
            blob_names += ['bbox_inside_weights_3rd']
            blob_names += ['bbox_outside_weights_3rd']
        ...
        if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_ROIS:
            ...
            # Same format as rois blob, but one per FPN level
            for lvl in range(k_min, k_max + 1):
                blob_names += ['rois_3rd_fpn' + str(lvl)]
            blob_names += ['rois_3rd_idx_restore_int32']
            if is_training:
...
def add_fast_rcnn_blobs(blobs, im_scales, roidb, stage_num):
    """Add blobs needed for training Fast R-CNN style models."""
    # Sample training RoIs from each image and append them to the blob lists
    for im_i, entry in enumerate(roidb):
        frcn_blobs = _sample_rois(entry, im_scales[im_i], im_i, stage_num)
    ...
    if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_ROIS:
        _add_multilevel_rois(blobs, stage_num)
...
def _sample_rois(roidb, im_scale, batch_idx, stage_num):
    ...
    bg_inds = np.where(
        (max_overlaps < bg_thresh_hi) &
        (max_overlaps >= bg_thresh_lo)
    )[0]
    ...
    if stage_num == 1:
        blob_dict = dict(
            labels_int32_1st=sampled_labels.astype(np.int32, copy=False),
    ...
    if stage_num == 2:
        blob_dict = dict(
            labels_int32_2nd=sampled_labels.astype(np.int32, copy=False),
    ...
    if stage_num == 3:
        blob_dict = dict(
            labels_int32_3rd=sampled_labels.astype(np.int32, copy=False),
...
def _add_multilevel_rois(blobs, stage_num):
    ...
    if stage_num == 1:
        _distribute_rois_over_fpn_levels('rois_1st')
    elif stage_num == 2:
        _distribute_rois_over_fpn_levels('rois_2nd')
    elif stage_num == 3:
        _distribute_rois_over_fpn_levels('rois_3rd')
        if cfg.MODEL.MASK_ON:
