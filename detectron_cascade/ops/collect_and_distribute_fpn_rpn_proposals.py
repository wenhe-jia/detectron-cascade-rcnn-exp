...
class CollectAndDistributeFpnRpnProposalsOp(object):
    ...
    def forward(self, inputs, outputs):
        ...
        #  + [roidb, im_info]
        # outputs is
        # [rois_1st, label_int32_1st, bbox_targets_1st, bbox_inside_weights, bbox_outside_weights,
        #  rois_1st_idx_restore_int32]
        ...
        if self._train:
            ...
            output_blob_names = fast_rcnn_roi_data.get_cascade_fast_rcnn_blob_names(is_training=True, stage_num=1)
            blobs = {k: [] for k in output_blob_names}
            fast_rcnn_roi_data.add_fast_rcnn_blobs(blobs, im_scales, roidb, 1)
            for i, k in enumerate(output_blob_names):
                blob_utils.py_op_copy_blob(blobs[k], outputs[i])

            # reset roidb for next rcnn stage, remove 'max_overlaps', 'gt_classes', 'bbox_targets' in each roidb,
