...
import detectron_cascade.utils.keypoints as keypoint_utils
import detectron_cascade.utils.segms as segm_utils
...
def combined_roidb_for_training(dataset_names, proposal_files):
    ...
    add_bbox_regression_targets(roidb, 0)
    logger.info('done')
...
def add_bbox_regression_targets(roidb, stage_num):
    ...
    for entry in roidb:
        entry['bbox_targets'] = compute_bbox_regression_targets(entry, stage_num)
...
def compute_bbox_regression_targets(entry, stage_num):
    ...
    targets[ex_inds, 1:] = box_utils.bbox_transform_inv(
        ex_rois, gt_rois, bbox_reg_weights)
    return targets
